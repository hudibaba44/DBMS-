...
def BookHotel1():
	...
	if(start_date<datenow.strftime('%Y-%m-%d')):
		...
		return render_template('bookhotel1.html', query_result = query_result, ac = ac, room_type = room_type, start_date = start_date, end_date = end_date)
	else:
...
def BookHotel2():
	...
	return render_template('Bookhotel2.html')

@app.route('/showHotelHistory')
def showHotelHistory():
	cursor.execute("SELECT hr.ID,h.name,h.city,Room_ID,Fare_AC,Cost,Start_Date,End_Date FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid".format(userinfo[0]))
	result=cursor.fetchall()
	print(result)
	cursor.execute("SELECT distinct h.name FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid".format(userinfo[0]))
	hotelname=cursor.fetchall()
	cursor.execute("SELECT distinct h.city FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid".format(userinfo[0]))
...
def filterhhistory():
	...
	if(filterh=="Hotel Name"):
		...
		cursor.execute("SELECT hr.ID,h.name,h.city,Room_ID,Fare_AC,Cost,Start_Date,End_Date FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid and h.name='{}' ".format(userinfo[0],data))
		result=cursor.fetchall()
	elif(filterh=="Date After"):
		data=request.form['sdate']
		print(data,type(data))
		cursor.execute("SELECT hr.ID,h.name,h.city,Room_ID,Fare_AC,Cost,Start_Date,End_Date FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid and Start_Date > '{}' ".format(userinfo[0],data))
		result=cursor.fetchall()
		...
		print(data,type(data))
		cursor.execute("SELECT hr.ID,h.name,h.city,Room_ID,Fare_AC,Cost,Start_Date,End_Date FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid and h.city='{}' ".format(userinfo[0],data))
		result=cursor.fetchall()
	elif(filterh=="StartDEndDate"):
		data=request.form['sdate']
		data2=request.form['edate']
		if(data<data2):
			cursor.execute("SELECT hr.ID,h.name,h.city,Room_ID,Fare_AC,Cost,Start_Date,End_Date FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid and Start_Date > '{}' and End_Date <'{}' ".format(userinfo[0],data,data2))
			result=cursor.fetchall()
		else:
			result=()
	elif(filterh=='HandC'):
		...
		cursor.execute("SELECT hr.ID,h.name,h.city,Room_ID,Fare_AC,Cost,Start_Date,End_Date FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid and h.city='{}' and h.name='{}'".format(userinfo[0],data2,data))
		result=cursor.fetchall()
	else:
		data=request.form['AC']
		cursor.execute("SELECT hr.ID,h.name,h.city,Room_ID,Fare_AC,Cost,Start_Date,End_Date FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid and Fare_AC='{}'".format(userinfo[0],data))
		result=cursor.fetchall()
	cursor.execute("SELECT distinct h.name FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid".format(userinfo[0]))
	hotelname=cursor.fetchall()
	cursor.execute("SELECT distinct h.city FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid".format(userinfo[0]))
...
def sortbytable():
	...
	if(data2=="Sort Ascending"):
		...
		sort="DESC"
	if(data=="Booking ID"):
		cursor.execute("SELECT hr.ID,h.name,h.city,Room_ID,Fare_AC,Cost,Start_Date,End_Date FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid order by(hr.ID) {}".format(userinfo[0],sort))
		result=cursor.fetchall()
		print(result)
	elif(data=="HotelName"):
		cursor.execute("SELECT hr.ID,h.name,h.city,Room_ID,Fare_AC,Cost,Start_Date,End_Date FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid order by(h.name) {}".format(userinfo[0],sort))
		result=cursor.fetchall()
		print(result)
	elif(data=="AC"):
		cursor.execute("SELECT hr.ID,h.name,h.city,Room_ID,Fare_AC,Cost,Start_Date,End_Date FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid order by(Fare_AC) {}".format(userinfo[0],sort))
		result=cursor.fetchall()
		print(result)
	elif(data=="Start Date"):
		cursor.execute("SELECT hr.ID,h.name,h.city,Room_ID,Fare_AC,Cost,Start_Date,End_Date FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid order by(Start_Date) {}".format(userinfo[0],sort))
		result=cursor.fetchall()
		print(result)
	elif(data=="End Date"):
		cursor.execute("SELECT hr.ID,h.name,h.city,Room_ID,Fare_AC,Cost,Start_Date,End_Date FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid order by(End_Date) {}".format(userinfo[0],sort))
		result=cursor.fetchall()
		print(result)
	elif(data=="City"):
		cursor.execute("SELECT hr.ID,h.name,h.city,Room_ID,Fare_AC,Cost,Start_Date,End_Date FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid order by(h.city) {}".format(userinfo[0],sort))
		result=cursor.fetchall()
		print(result)
	else:
		cursor.execute("SELECT hr.ID,h.name,h.city,Room_ID,Fare_AC,Cost,Start_Date,End_Date FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid order by(Cost) {}".format(userinfo[0],sort))
		result=cursor.fetchall()
		print(result)
	cursor.execute("SELECT distinct h.name FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid".format(userinfo[0]))
	hotelname=cursor.fetchall()
...
def cancelhotel():
	...
	result=()
	print(datenow)
	cursor.execute("SELECT hr.ID,h.name,h.city,Room_ID,Fare_AC,Cost,Start_Date,End_Date FROM Hotel_Reservation hr NATURAL JOIN User1 u NATURAL JOIN Hotel h,Fare_Hotel where hr.User_ID={} and Fare_Rid=Room_ID and Fare_Hid=Hid and Start_Date > '{}'".format(userinfo[0],datenow.strftime('%Y-%m-%d')))
	result=cursor.fetchall()
	print(result)
